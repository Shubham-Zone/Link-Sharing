...
const CardComponent = ({ id, img, user, name, createdBy, visibility, type }) => {
    ...
    useEffect(() => {
        const countSubscriptions = async () => {
            try {
                const res = await axios.get(`http://localhost:8000/api/subscription-count/${id}`, {
                    headers: {
                        "x-auth-token": token,
                    }
                });
    ...
    const subscribeTopic = async () => {
        ...
        try {
            ...
            console.log(user);
            if (res.status === 200) {
                alert(`Subscribed to topic ${name} successfully`);
                    ...
                    <div className="modal-dialog">
                        <div className="modal-content">
                            ...
                            <div className="modal-footer">
                                ...
                                <button onClick={() => deleteTopic()} type="button" className="btn btn-primary" data-bs-dismiss="modal">Delete</button>
                            </div>
                        </div>
                    </div>
                </div>
