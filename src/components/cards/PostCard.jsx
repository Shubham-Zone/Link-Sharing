...
const TopicUserCard = ({ topic, topics, setTopics, type }) => {
    ...
    useEffect(() => {
        ...
        const handleCountSubscriptions = async () => {
            try {
                const res = await countSubscriptions(topic._id);
                if (res.status === 200) {
        ...
        const handleCountPosts = async () => {
            try {
                const res = await countPosts(topic._id);
                if (res.status === 200) {
            ...
            <div className="row g-0">
                ...
                <div className="col-md-8">
                    <div className="card-body">
                        ...
                        <p className="card-text text-muted">{topic.createdBy}</p>
                        <div className="d-flex justify-content-between">
                            <p className="mb-0"><strong>Posts:</strong> {posts}</p>
                            <p className="mb-0"><strong>Subscriptions:</strong> {count}</p>
                        </div>
                        <div className="d-flex justify-content-between">
                            <p className="mb-0"><strong>Visibility:</strong> {topic.visibility}</p>
                        </div>
                        <br />
                        <div class="row g-3 align-items-center">
                                    ...
                                    <div class="d-flex gap-2">
                                        {uuid === topic.createdBy && (
                                            <>
                                                ...
                                                ></i>
                                                <i onClick={() => toggle()} class="bi bi-trash text-danger" style={{ fontSize: '1.3rem' }}></i>
                                            </>
                                        )}
                                    </div>
                                </div>
                            ...
                            <div className="d-flex col gap-2">
                                ...
                                <i onClick={togglePostModal} class="bi bi-file-earmark-plus-fill" style={{ fontSize: '2.0rem' }}></i>
                                <Link to={`/posts/${topic._id}`} className="btn btn-primary">Posts</Link>
                            </div>
                        </div>
